...
Print a image received from Serial.
With this example you can print an image just calling the sendImage.py (read the
README.txt inside 'extras' folder first). In this sketch you don't need change
anything, only you need to do in this sketch is adjust the correct pins (if you are using
different wiring than itself.)
...
#define STEPS_PER_PIXEL_COL  130

String lineFromSerial = "";    //string to buffering data from serial
bool stringComplete   = false; //flag used in serial interrupt to sinalize data to print
...
void setup(){
  ...
  printer.begin(FULL_STEP, FULL_STEP, SERVO_DEGREES, SERVO_PIN, STEPS_PER_PIXEL_COL, STEPS_PER_PIXEL_LINE);

}

void loop(){
    if (stringComplete){
        char buf[lineFromSerial.length()] = {0};
        lineFromSerial.toCharArray(buf,lineFromSerial.length()+1);
